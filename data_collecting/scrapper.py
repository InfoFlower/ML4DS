...
def main(url):
    ...
    for timestamp, original_url in archives:
        if timestamp not in existing_files and timestamp[:8] not in [i[:8] for i in existing_files]:
            download_archive(timestamp, original_url, download_dir)
            time.sleep(10)  # Be nice to the Wayback Machine
        elif timestamp[:8] not in [i[:8] for i in existing_files]: print(f'{timestamp} already downloaded')
        else: print(f'{timestamp[:8]} Same date already downloaded')
